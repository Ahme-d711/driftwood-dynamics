    ...
    <DashboardLayout>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-border/30">
                  {isLoading && (
                    <tr><td colSpan={7} className="py-10 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-accent" /></td></tr>
                  )}
                  {isError && (
                    <tr><td colSpan={7} className="py-10 text-center text-sm text-destructive">Failed to load orders.</td></tr>
                  )}
                  {!isLoading && !isError && orders.length === 0 && (
                    <tr><td colSpan={7} className="py-10 text-center text-sm text-muted-foreground">No orders found.</td></tr>
                  )}
                  {orders.map((order) => (
                    <tr key={order._id} className="group hover:bg-muted/30 transition-colors">
                      ...
                      <td className="py-4"><Badge variant="secondary" className={`${getStatusColor(order.status)} border-none`}>{order.status}</Badge></td>
                      <td className="py-4 text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="icon" title="View Details" onClick={() => openViewDialog(order._id)}><Eye className="w-4 h-4" /></Button>
                          <Button variant="ghost" size="icon" title="Update Status" onClick={() => openEditDialog(order._id, order.status)}><Truck className="w-4 h-4 text-accent" /></Button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button onClick={() => selectedOrderId && updateStatusMutation.mutate({ id: selectedOrderId, value: status })} disabled={updateStatusMutation.isPending}>
